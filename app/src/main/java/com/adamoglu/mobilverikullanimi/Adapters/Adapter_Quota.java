...
public class Adapter_Quota extends ArrayAdapter<String> {
    ...
    public Adapter_Quota(Context context, ArrayList values_name, ArrayList values_uid, ArrayList values_icon, ArrayList color) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        checkbox.setChecked(SharedPreferences().getBoolean("check"+values_name.get(position),false));
        ...
        if(res.getCount()!=0)
            while (res.moveToNext()){
                if(res.getString(0).equals(values_name.get(position))){
        ...
        checkbox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if(checkbox.isChecked()){
                    myDb.Add_Quota(values_uid.get(position));
                    Editor().putBoolean("check"+values_name.get(position),true).commit();

                    Service_Data servicedata = new Service_Data();
                    servicedata.ResApp();
                }

                else{
                    myDb.DeleteData_Quota(String.valueOf(values_uid.get(position)));
                    Editor().putBoolean("check"+values_name.get(position),false).commit();
                }

                ServiceApps_Test serviceApps_test = new ServiceApps_Test(getContext());
                serviceApps_test.getQuotaApps();


                Cursor res = myDb.getAllData_Quota();
                Editor().putString("QuataAppSize", String.valueOf(res.getCount())).commit();
